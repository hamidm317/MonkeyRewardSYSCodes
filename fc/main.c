...
int randomSequenseNumbersChoice[16];
int randomSequenseNumbersRegionsChoice[16];
int randomSequenseNumbersForce[64];
int numberOfFractals;
int currentSequenceNumber;
//int numberOfBatchFractals;
int numberOfBatchExperiments;
...
///*
	...
	//SDL_Surface* screenSurface = NULL;
		...
		printf("displays = %d\n,3: bound x = %d, bound y = %d\n, bound3 w = %d, bound3 h =%d\n", displays, bound3.x, bound3.y, bound3.w, bound3.h);
		...
		window = SDL_CreateWindow( "Force and Choice Task",bound1.x, bound1.y, bound1.w, bound1.h, SDL_WINDOW_BORDERLESS);
			//printf( "After SDL window 1\n");
			//printf( "before rend\n");
			renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
			//printf( "after rend\n");
		window2 = SDL_CreateWindow( "Force and Choice Task",bound2.x, bound2.y, bound2.w, bound2.h, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
  ...
  eyecmd_printf("link_sample_data  = LEFT,RIGHT,GAZE,GAZERES,AREA,STATUS%s,INPUT",(tracker_software_ver>=4)?",HTARGET":"");
	...
	*/
	...
	printf("Saving trialID = %d to file\n" , trialID);
        ...
        strcat (our_file_name_final, "_FC");
...
srand(time(NULL));
	...
	trialStates[Trial_State_Fixation_Fix].expirationTime = 400;
	trialStates[Trial_State_Fract_Overlap].expirationTime = 400;
	trialStates[Trial_State_Wait_For_Saccade].expirationTime = 3000;
	trialStates[Trial_State_Fractal_Fix].expirationTime = 400;
	...
	Regions_Inter_Ring_Distance = degToPixel(7.0);
        ...
        Rect_Sensation.w = (int)degToPixel(6.0);
