...
SDL_Texture *Texture_Edit_Overlay = NULL;
...
//SDL_Point Point_Bounds_Circle_Sector[8];
float renderer2ScaleX_inv;
float renderer2ScaleY_inv;
...
/*	if( SDL_Init( SDL_INIT_VIDEO ) < 0 ){
	...
	//SDL_Surface* screenSurface = NULL;
		...
		*/
		...
		window = SDL_CreateWindow( "RF Task",bound1.x, bound1.y, bound1.w, bound1.h, SDL_WINDOW_BORDERLESS);
			//printf( "After SDL window 1\n");
			//printf( "before rend\n");
			renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
			//printf( "after rend\n");
		window2 = SDL_CreateWindow( "RF Task",bound2.x, bound2.y, bound2.w, bound2.h, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
			...
			Texture_Edit_Overlay = SDL_CreateTexture(renderer2, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, bound2.w, bound2.h);
			SDL_SetTextureBlendMode(Texture_Edit_Overlay, SDL_BLENDMODE_BLEND);
			//printf( "after rend2\n");
...
//*
	renderer2ScaleX_inv = (float) bound2.w / (float) bound1.w;
	renderer2ScaleY_inv = (float) bound2.h / (float) bound1.h;
	...
	Color_Edit_Overlay.r = 32;
	Color_Edit_Overlay.g = 32;
	Color_Edit_Overlay.b = 32;
	Color_Edit_Overlay.a = 255;
  ...
  eyecmd_printf("link_sample_data  = LEFT,RIGHT,GAZE,GAZERES,AREA,STATUS%s,INPUT",(tracker_software_ver>=4)?",HTARGET":"");
	...
	*/
	...
	printf("trialID : %d\n", trialID);
        ...
        strcat (our_file_name_final, "_RF");
...
/*
		...
		*/
		...
		else if(_stricmp(argv[i],"-numoffracts") ==0 && argv[i+1])
                {
                        i++;
                        numberOfBatchFractals = atoi(argv[i]);
                }
		/*
                ...
                else if(_stricmp(argv[i],"-fractoff") ==0 && argv[i+1])
                {
                        i++;
                        trialStates[Trial_State_Fract_Off].expirationTime = atoi(argv[i]);
                }
                else if(_stricmp(argv[i],"-fracton") ==0 && argv[i+1])
                {
                        i++;
                        trialStates[Trial_State_Fract_On].expirationTime = atoi(argv[i]);
                }
		else if(_stricmp(argv[i],"-fixsize") ==0 && argv[i+1])
                {
                        i++;
                        Rect_Fixation_Monkey.w = degToPixel(atof(argv[i]));
                        Rect_Fixation_Monkey.h = degToPixel(atof(argv[i]));
                }
                else if(_stricmp(argv[i],"-fixwin") ==0 && argv[i+1])
                {
                        i++;
                        Rect_Fixation_Window.w = degToPixel(atof(argv[i]));
                        Rect_Fixation_Window.h = degToPixel(atof(argv[i]));
                }
                else if(_stricmp(argv[i],"-radius") ==0 && argv[i+1])
                {
                        i++;
                        Regions_Inter_Ring_Distance = degToPixel(atof(argv[i]));
                }
                else if(_stricmp(argv[i],"-fractsize") ==0 && argv[i+1])
                {
                        i++;
                        Regions_Width = degToPixel(atof(argv[i]));
                        Regions_Height = degToPixel(atof(argv[i]));
                }
		...
		else
		{
			...
			*/
			...
			printf("\t === General ===\n");
                        printf("\t\t -numoftrials <number of trials> (max 256). currently %d \n", numberOfFractals);
                        printf("\t\t -numoffracts <number of fractals in each trial>. currently %d \n", numberOfBatchFractals);
                        /*
			printf("\t\t -set <fractal set>. currently %d \n", fractSetToLoad);
                        ...
                        printf("\t\t -fractoff <duration of fractal off in ms>. currently %d \n", trialStates[Trial_State_Fract_Off].expirationTime);
                        printf("\t\t -fracton <duration of fractal on in ms>. currently %d \n", trialStates[Trial_State_Fract_On].expirationTime);
                        printf("\t === Fractal ===\n");
                        printf("\t\t -fixsize <size of fixation in pixels>. currently %f \n", pixelToDeg(Rect_Fixation_Monkey.w));
                        printf("\t\t -fixwin <size of fixation window in pixels>. currently %f \n", pixelToDeg(Rect_Fixation_Window.w));
                        printf("\t\t -radius <radius to display fractals from center in pixels>. currently %f \n", pixelToDeg(Regions_Inter_Ring_Distance));
                        printf("\t\t -fractsize <size of fractal in pixels>. currently %f \n", pixelToDeg(Regions_Width));
                        printf("\t === Sound ===\n");
...
int main(int argc, char ** argv)
{
	...
	fractSetToLoad = defaultFractalSet;

        Regions_Inter_Ring_Distance = (int)degToPixel(2.6);
        Regions_Offset = 0;
        Regions_Width = (int)degToPixel(4.0);
        Regions_Height = (int)degToPixel(4.0);
        ...
        Rect_Fixation_Window.w = (int)degToPixel(6.0);
        Rect_Fixation_Window.h = (int)degToPixel(6.0);
        Rect_Gaze.w = (int)degToPixel(0.25);
        Rect_Gaze.h = (int)degToPixel(0.25);
	    Rect_Sensation.w = (int)degToPixel(4.0);
		Rect_Sensation.h = (int)degToPixel(4.0);
        numberOfFractals = 255;
	numberOfBatchFractals = 8;

        errorVolume = 0.4;
        successVolume = 0.5;

	trialStates[Trial_State_ITI].expirationTime = 2000;
	trialStates[Trial_State_Wait_For_Gaze].expirationTime = 3000;
	/*trialStates[Trial_State_Fract_Off].expirationTime = 200;
	trialStates[Trial_State_Fract_On].expirationTime = 200;*/
	...
	if (fractSetToLoad == defaultFractalSet){
		...
		trialStates[Trial_State_Fract_Off].expirationTime = 400;
		trialStates[Trial_State_Fract_On].expirationTime = 400;
	}
	/*
