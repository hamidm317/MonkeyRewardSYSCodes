...
SDL_Surface *Surface_Fractals[64];
SDL_Texture *Texture_Fractals[64];
SDL_Texture *Texture_Fractals_Monkey[64];
...
int parseArgs(int argc, char **argv, char **trackerip, DISPLAYINFO *disp )
{
	...
	{	
		...
		if (_stricmp(argv[i],"-set") ==0 && argv[i+1]){
			i++;
			setsToLoad[0] = atoi(argv[i]);
			setsToLoad[1] = atoi(argv[i]) + 1;
			setsToLoad[2] = atoi(argv[i]) + 2;
			setsToLoad[3] = atoi(argv[i]) + 3;
			setsToLoad[4] = atoi(argv[i]) + 4;
			setsToLoad[5] = atoi(argv[i]) + 5;
			setsToLoad[6] = atoi(argv[i]) + 6;
			setsToLoad[7] = atoi(argv[i]) + 7;
			printf("sets = %d %d %d %d %d %d %d %d\n", setsToLoad[0], setsToLoad[1], setsToLoad[2], setsToLoad[3], setsToLoad[4], setsToLoad[5], setsToLoad[6], setsToLoad[7]);
		}
	}	
	return 0;
}
...
void fractalLoaderFromSets(){
        //101 , 102
        //set -= 100;
	printf("Start HERE!");

        int *fractSetIdx = {0, 0, 0, 0, 0, 0, 0, 0};
	int k = 0;
	
	for (int i = 0; i < 8; i++)
	{

		fractSetIdx[i] = setsToLoad[i] * 10;
        ...
        for (int i=0; i<8; i++)
	{
		...
		if (setsToLoad[i] > 120 && setsToLoad[i] <= 200)
		{

			for (int j = 0; j < 8; j++)
			{
				
				sprintf(idxToStr, "%d", fractSetIdx[i] + j + 1);
				strcat(temp, idxToStr);
                		strcat(temp, ".jpeg");
				Surface_Fractals[k] = IMG_Load(temp);
				printf("Loading Fractal #%d : %s\n", k, temp);
				Texture_Fractals[k] = SDL_CreateTextureFromSurface(renderer2, Surface_Fractals[k]);
				SDL_SetTextureBlendMode(Texture_Fractals[k], SDL_BLENDMODE_BLEND);
				Texture_Fractals_Monkey[k] = SDL_CreateTextureFromSurface(renderer, Surface_Fractals[k]);
				SDL_SetTextureBlendMode(Texture_Fractals_Monkey[k], SDL_BLENDMODE_BLEND);
				k++;
		...
		else
		{
			...
			sprintf(idxToStr, "%d", fractSetIdx + 1);
			strcat(temp, idxToStr);
                	strcat(temp, ".jpeg");
			Surface_Fractals[k] = IMG_Load(temp);
			printf("Loading Fractal #%d : %s\n", k, temp);
			...
			sprintf(idxToStr, "%d", fractSetIdx + 5);
			strcat(temp, idxToStr);
                	strcat(temp, ".jpeg");
			Surface_Fractals[k] = IMG_Load(temp);
			printf("Loading Fractal #%d : %s\n", k, temp);
